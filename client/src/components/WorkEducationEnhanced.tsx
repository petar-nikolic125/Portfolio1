/* ────────────────────────────────────────────────────────────────
   WorkEducationEnhanced.tsx · ultra-polished dual timeline
   Pure React + Tailwind (no zustand / redux)
   Drops straight into <Home />                Updated 2025-06-29
──────────────────────────────────────────────────────────────── */
...
interface Base {
  ...
  subHeading: string;
  subText?: string;
  logo?: string;      // optional /public/... image -- leave blank to show emoji
...
function Badge({
  ...
  const [loaded, setLoaded] = useState(!logo); // if no logo, already "loaded"
  ...
  return (
      ...
      {!loaded && (
        ...
        `}
            ...
            onError={(e) => {
              /* @ts-ignore runtime swap */
              e.currentTarget.replaceWith(fallbackIcon);
            }}
        />
    </span>
  );
...
export default function WorkEducationEnhanced() {
  ...
  /* stagger animation every switch */
  useEffect(() => {
  ...
  return (
      ...
      >
        ...
        {item.ribbon && (
            <span
                className={`
            absolute -top-1 -right-1 rounded px-1.5 py-[1px]
            text-[9px] font-bold
            ${
                    item.ribbon === 'LIVE'
